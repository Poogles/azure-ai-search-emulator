...
- Per-query `weight` scales that vector query's contribution to the hybrid RRF fusion (a heavier query lifts its matches); `sessionId` is accepted but inert. A missing vector-query `kind` defaults to `"vector"` (emulator-only leniency).
...
- Knowledge sources and bases are stored, echoed, and managed (create/update/get/list/delete) but **inert**: no ingestion, synchronization, or model inference runs. `POST /knowledgebases('{name}')/retrieve` performs no model inference; instead it returns the documents from the base's `searchIndex` knowledge sources (each tagged with an `@search.source` field), searched with the request's search text (top-level `query`, or the first intent's `search` as sent by the SDKs; otherwise a match-all) and limited by `top` (default 3, max 1000). `activity` and `references` remain empty. A missing source index fails the call with `404 ResourceNotFound`.
- Etags are opaque counter strings, not Azure's hex entity tags.
- **Rationale:** the CRUD surface is implemented so samples and clients that manage these resources work unchanged; model inference and agentic generation are initial-design non-goals, so retrieve returns raw source documents rather than a RAG pipeline's citations and generated activity.
